...
public class NaverManager {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(NaverManager.class);
	
	private static final String REDIRECT_URI = "/login/sns/naver.do";
	...
	public String getAccessToken(HttpServletRequest request) {
		...
		String naverUse = snsProp.getProperty("sns.naver.use");
		
		//SNS 사용중이 아니면 false
		if(!StringUtil.isEqual(snsUse, "Y") || !StringUtil.isEqual(naverUse, "Y")) {
			return null;
		}

		//KAKAO API키
		String apiKey = snsProp.getProperty("sns.naver.apiKey");
		...
		try {
			...
			String apiURL = "https://kauth.naver.com/oauth/token";
	...
	public UserVO getUserInfo(String accessToken) {
		...
		try {
	        ...
	        String apiURL = "https://kapi.naver.com/v2/user/me";
            url = new URL(apiURL);
            ...
            if(responseCode==200) {
				...
				if(userMap != null) {
					String userId = "naver@" + userMap.get("id");
					user = new UserVO();
