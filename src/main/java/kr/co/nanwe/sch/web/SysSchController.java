...
@RequestMapping(value = "/sys/sch")
@Program(code="COM_SCH", name="일정관리")
@Controller
public class SysSchController {
	
	//private static final Logger LOGGER = LoggerFactory.getLogger(SysSchController.class);
	
	/** View Path */
	private static final String VIEW_PATH = "sys/sch";
	...
	/** Constructor */
	public SysSchController() {		
		RequestMapping requestMapping = SysSchController.class.getAnnotation(RequestMapping.class);
	...
	public String registerAction(Model model, HttpServletRequest request, @ModelAttribute SearchVO search
		...
		if(result > 0) { //성공시
			...
			Map<String, Object> resultParam = new HashMap<String, Object>();
			resultParam.put("sCode", schVO.getSchCd());
	...
	public String modifyView(Model model, HttpServletRequest request, @ModelAttribute SearchVO search
		...
		boolean result = (boolean) scheduleMap.get("result");
		if(!result) {
