		...
		<c:if test='${bbsMgtVO.noticeYn eq "Y" and !empty noticeList }'>
			<c:forEach var="item" items="${noticeList }" varStatus="i">
				<tr class="notice">
					...
					<td class="subj">
						<div class="tit">
							<a class="link" href="/board/<c:out value='${fn:toLowerCase(sCode)}/view.do?sId=${item.bbsId }&cp=${search.cp }&sv=${search.sv }&so=${search.so }&sCate=${sCate }'/>" title="<spring:message code="board.detail"/>">
