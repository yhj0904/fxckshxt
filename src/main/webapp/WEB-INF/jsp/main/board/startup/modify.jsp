...
<table class="detail_table">
	...
	<tr>
		...
		<td>
			<input id="title" name="title" type="text" value="<c:out value='${bbsVO.title }' escapeXml='false'/>">
	...
	<c:if test='${bbsMgtVO.supplYn eq "Y" }'>
		<c:if test='${bbsMgtVO.suppl01Yn eq "Y" }'>
			<tr>
				...
				<td><form:select path="bbsInfo01" id="comboAll" >
									<c:forEach var="cate" items="${companyCode.list }">
										<form:option value="${cate.cd }" label="${cate.cdNm }" />
									</c:forEach>
								</form:select><span class="form_error" data-path="bbsInfo01"><form:errors path="bbsInfo01" /></span></td>
			</tr>
		</c:if>
