		...
		<td>
			<c:if test="${!empty bbsVO.viewFiles }">
				<c:forEach var="viewFile" items="${bbsVO.viewFiles }">
					<c:if test='${viewFile.fext eq "mp4" }'>
						<div>
							<video controls="controls" style="width:100%; height:auto;" title="${viewFile.oname }">
								<source src="/file/videoView.do?fpath=<c:out value='${viewFile.fpath }'/>&fname=<c:out value='${viewFile.fname }'/>&fmime=<c:out value='${viewFile.fmime }'/>" type='video/mp4; codecs="avc1.42E01E, mp4a.40.2"' />
							</video>
						</div>
					</c:if>
				</c:forEach>
