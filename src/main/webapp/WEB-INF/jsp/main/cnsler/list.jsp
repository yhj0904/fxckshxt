...
<%--			</c:if>--%>
			<option value="cnslUserNm">신청자 이름</option>
		</select>
...
<form:form id="listForm" name="listForm" method="post" autocomplete="off">
	...
	<table class="list_table2">
		<colgroup>
			...
			<col style="width: 35%"/> <!-- 상담제목 -->
			<col style="width: 25%"/> <!-- 상담번호 -->
			<col style=""/> <!-- 상담분야 -->
			<col style=""/> <!-- 상담방법 -->
			<col style=""/> <!-- 상담신청일 -->
			<col style=""/> <!-- 진행상태 -->
...
<%--					<td><a class="link" href="javascript:fn_detailView('<c:out value="${item.CNSL_ID}" />');" ><c:out value="${item.CNSL_ID}"/></a></td>--%>
					<td><a class="link" href="/cnsler/view.do?sId=<c:out value="${item.CNSL_ID}" />" ><c:out value="${item.CNSL_ID}"/></a></td>
					...
					%>
					<td><c:out value="${deptCdVO.deptNmKor}"/></td> --%>
					...
					<td><c:out value="${item.REQST_DT}" /></td>
				</tr>
			</c:forEach>
   	...
   	<div class="btn_wrap">
		<ul>
			<li><a class="button register_btn" href="javascript:fn_registerView();">상담 <spring:message code="button.register" /></a></li>
