...
<form name="boardSearchForm" class="board_search_form" method="post" autocomplete="off">
	<div class="search_box">
		...
		<select name="so" title="<spring:message code="search.choose"/>">
			<option value="bannerCd" <c:if test="${search.so eq 'bannerCd'}">selected</c:if>><spring:message code="bannerVO.bannerCd"/></option>
			<option value="bannerNm" <c:if test="${search.so eq 'bannerNm'}">selected</c:if>><spring:message code="bannerVO.bannerNm"/></option>
		</select>		
	...
	<table class="list_table">
		...
		<tbody>
			<c:forEach var="item" items="${list }" varStatus="i">
				<tr>
					<td><label><input type="checkbox" name="checkRow" value="<c:out value="${item.bannerId }"/>" onclick="fn_checkRow();"><span class="none"><spring:message code="board.check" /></span></label></td>
					<td><c:out value="${(paging.totalCount - (paging.currentPage -1) * paging.pageSize) - i.index}"/></td>
					<td><c:out value="${item.bannerCd}"/></td>
					<td><a class="link" href="javascript:fn_detailView('<c:out value="${item.bannerId }"/>');" title="<spring:message code="board.detail"/>"><c:out value="${item.bannerNm}"/></a></td>
					<td><c:out value="${item.bannerLink}"/></td>
					<td><c:out value="${item.startDttm}"/></td>
					<td><c:out value="${item.endDttm}"/></td>
