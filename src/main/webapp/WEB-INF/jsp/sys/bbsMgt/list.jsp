...
/**
 * @Class Name 	: list.jsp
 * @Description : 게시판관리 목록 화면
 * @Modification Information
 * @
 * @ 수정일			수정자			수정내용
 * @ --------------------------------------------------------------------------
 * @ 2020.01.06		임문환			최초생성
 * @ 2023.10.26		신한나			채용공고 바로가기 사용자페이지로 가도록 변경.
 */
%>
<form name="boardSearchForm" class="board_search_form" method="post" autocomplete="off">
	<div class="search_box">
		...
		<select name="so" title="<spring:message code="search.choose"/>">
			<option value="code" <c:if test="${search.so eq 'code'}">selected</c:if>><spring:message code="bbsMgtVO.code"/></option>
			<option value="title" <c:if test="${search.so eq 'title'}">selected</c:if>><spring:message code="bbsMgtVO.title"/></option>
		</select>		
		...
		<tbody>
			<c:forEach var="item" items="${list }" varStatus="i">
				<tr>
					...
					<td><c:out value="${item.pageCnt}"/></td>
					<td><c:out value="${item.rowCnt}"/></td>
					<td><c:out value="${item.inptDttm}"/></td>
					<td>
						<c:choose>
							<c:when test="${item.code eq 'BOARD06' }">
								<a class="table_btn" href="/emp.do"><spring:message code="button.shortcut"/></a>
							</c:when>
							<c:when test="${item.code eq 'BOARD09' }">
								<a class="table_btn" href="/labr.do"><spring:message code="button.shortcut"/></a>
							</c:when>
							<c:otherwise>
								<a class="table_btn" href="javascript:fn_bbsView('<c:out value="${item.code }"/>');"><spring:message code="button.shortcut"/></a>
							</c:otherwise>
						</c:choose>
					</td>
...
<script type="text/javaScript" defer="defer">
	...
	//수정
	function fn_bbsView(sCode) {
		document.listForm.sCode.value = sCode;
		document.listForm.action = "/sys/bbs.do";
		document.listForm.submit();
	}
