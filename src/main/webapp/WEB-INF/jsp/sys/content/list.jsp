...
<form name="boardSearchForm" class="board_search_form" method="post" autocomplete="off">
	<div class="search_box">
		<select name="so" title="<spring:message code="search.choose"/>">
			<option value="contId" <c:if test="${search.so eq 'contId'}">selected</c:if>><spring:message code="contentVO.contId"/></option>
			<option value="contNm" <c:if test="${search.so eq 'contNm'}">selected</c:if>><spring:message code="contentVO.contNm"/></option>
		</select>		
	...
	<table class="list_table">
		...
		<tbody>
			<c:forEach var="item" items="${list }" varStatus="i">
				<tr>
					<td><label><input type="checkbox" name="checkRow" value="<c:out value="${item.contId }"/>" onclick="fn_checkRow();"><span class="none"><spring:message code="board.check" /></span></label></td>
					<td><c:out value="${(paging.totalCount - (paging.currentPage -1) * paging.pageSize) - i.index}"/></td>
					<td><a class="link" href="javascript:fn_detailView('<c:out value="${item.contId }"/>');" title="<spring:message code="board.detail"/>"><c:out value="${item.contId}"/></a></td>
