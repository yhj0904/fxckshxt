...
<form name="boardSearchForm" class="board_search_form" method="post" autocomplete="off">
	<div class="search_box">
		...
		<select name="so" title="<spring:message code="search.choose"/>">
			<option value="popNm" <c:if test="${search.so eq 'popNm'}">selected</c:if>><spring:message code="popupVO.popNm"/></option>
		</select>		
	...
	<table class="list_table">
		...
		<tbody>
			<c:forEach var="item" items="${list }" varStatus="i">
				<tr>
					<td><label><input type="checkbox" name="checkRow" value="<c:out value="${item.popId }"/>" onclick="fn_checkRow();"><span class="none"><spring:message code="board.check" /></span></label></td>
					<td><c:out value="${(paging.totalCount - (paging.currentPage -1) * paging.pageSize) - i.index}"/></td>
					<td><a class="link" href="javascript:fn_detailView('<c:out value="${item.popId }"/>');" title="<spring:message code="board.detail"/>"><c:out value="${item.popId}"/></a></td>
					<td><c:out value="${item.popType}"/></td>
					<td><c:out value="${item.popNm}"/></td>
