	...
	<table class="list_table fixed">
		...
		<tbody>
			<c:forEach var="item" items="${list }" varStatus="i">
				<tr>
					<c:choose>
						<c:when test='${(loginInfo.statDvcd eq "SYS_ADM") and (item.PROG_REQST_CD eq "PROG_REQST_010") or (item.PROG_REQST_CD eq "PROG_REQST_090")}'>
							<td>
					...
					<td>
						<c:choose>
					        <c:when test="${fn:length(item.EMAIL) > 10}">
					            <c:set var="trimmedPart" value="${fn:substring(item.EMAIL, 0, 10)}" />
					            <c:out value="${trimmedPart}...."/>
					        </c:when>
					        <c:otherwise>
					...
					<td>
						<c:choose>
					        <c:when test="${fn:length(item.DEPT_NM) > 8}">
					            <c:set var="trimmedPart" value="${fn:substring(item.DEPT_NM, 0, 8)}" />
					            <c:out value="${trimmedPart}...."/>
					        </c:when>
					        <c:otherwise>
...
<script type="text/javaScript" defer="defer">
	...
	function fn_excelDown(progId){
		document.listForm.progId.value = progId;
		document.listForm.action = "/sys/prog/apply/excelDown.do";
		document.listForm.submit();
	}
