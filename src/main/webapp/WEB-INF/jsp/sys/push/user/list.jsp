...
<form name="boardSearchForm" class="board_search_form" method="post" autocomplete="off">
	<div class="search_box">
		<select name="so" title="<spring:message code="search.choose"/>">
			<option value="userId" <c:if test="${search.so eq 'userId'}">selected</c:if>><spring:message code="pushAppUserVO.userId"/></option>
			<option value="userNm" <c:if test="${search.so eq 'userNm'}">selected</c:if>><spring:message code="pushAppUserVO.userNm"/></option>
		</select>		
	...
	<table class="list_table">
		...
		<tbody>
			<c:forEach var="item" items="${list }" varStatus="i">
				<tr>
					<td><label><input type="checkbox" name="checkRow" value="<c:out value="${item.userId }"/>" onclick="fn_checkRow();"><span class="none"><spring:message code="board.check" /></span></label></td>
					...
					<td><c:out value="${item.deviceId}"/></td>
   	...
   	<div class="btn_wrap">
		<ul>
			...
			<li>
				<a class="button" href="javascript:fn_syncUser();">사용자 동기화</a>
			</li>
			<li>
...
<script type="text/javaScript" defer="defer">
	...
	//사용자동기화
	function fn_syncUser() {
		var msg = confirm('사용자를 동기화하시겠습니까?');
		if(msg == true){				
			document.listForm.action = GV_PRESENT_PATH + "/syncUser.do";
			document.listForm.submit();
		}
	}
