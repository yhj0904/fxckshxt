 ...
 */
%>
<form:form modelAttribute="survMgtVO" id="detailForm" name="detailForm" method="post" autocomplete="off">
	<ul class="sub_ul">
		<li>
			<div class="sub_cont">
		...
		<li>
			<div class="sub_tit"><spring:message code='survMgtVO.message.trgt'/></div>
			<div class="sub_cont">
				<table class="list_table">
					...
					<thead>
						<tr>
							...
							<th><spring:message code="survTrgtVO.trgtType"/></th>
							<th><spring:message code="survTrgtVO.trgtTarget"/></th>
						</tr>
					</thead>
					<tbody id="trgtList">
						<c:forEach var="trgt" items="${survMgtVO.trgtList }" varStatus="t">
							<c:set var="trgtCount" value="${t.count }"/>
							<tr>
								<td><span class="trgt_count"><c:out value='${t.count }'/></span></td>
								<td>
									<c:choose>
										<c:when test='${trgt.trgtType eq "TRGT_CD"}'><spring:message code="survTrgtVO.trgtTarget.cd"/></c:when>
										<c:when test='${trgt.trgtType eq "TRGT_ID"}'><spring:message code="survTrgtVO.trgtTarget.id"/></c:when>
									</c:choose>
								</td>
								<td>
									<c:choose>
										<c:when test='${trgt.trgtType eq "TRGT_CD"}'><c:out value='${trgt.trgtCd }'/></c:when>
										<c:when test='${trgt.trgtType eq "TRGT_ID"}'><c:out value='${trgt.trgtId }'/></c:when>
									</c:choose>
								</td>
							</tr>
						</c:forEach>
													...
													<tbody class="item_list">
														<c:forEach var="item" items="${question.itemList }" varStatus="j">
															<tr>
																...
																<td><c:out value="${item.itemEtc}"/></td>
		...
		<ul>
			<c:choose>					
				<c:when test='${survMgtVO.survState eq "N"}'>
					...
					<li><a class="button" href="javascript:fn_modifyState('<c:out value="${survMgtVO.survId }"/>', 'S');"><spring:message code="survMgtVO.message.start" /></a></li>
				</c:when>
				<c:when test='${survMgtVO.survState eq "S"}'>
					<li><a class="button" href="javascript:fn_modifyState('<c:out value="${survMgtVO.survId }"/>', 'E');"><spring:message code="survMgtVO.message.end" /></a></li>
				</c:when>
				<c:when test='${survMgtVO.survState eq "E"}'>
					<li><a class="button" href="javascript:fn_resultView('<c:out value="${survMgtVO.survId }"/>');"><spring:message code="survMgtVO.message.result" /></a></li>
				</c:when>
			</c:choose>			
