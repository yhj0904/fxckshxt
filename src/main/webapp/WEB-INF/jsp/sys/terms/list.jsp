...
<form name="boardSearchForm" class="board_search_form" method="post" autocomplete="off">
	<div class="search_box">
		<select name="so" title="<spring:message code="search.choose"/>">
			<option value="termsNm" <c:if test="${search.so eq 'termsNm'}">selected</c:if>><spring:message code="termsVO.termsNm"/></option>
		</select>		
	...
	<table class="list_table">
		...
		<tbody>
			<c:forEach var="item" items="${list }" varStatus="i">
				<tr>
					<td><label><input type="checkbox" name="checkRow" value="<c:out value="${item.termsId }"/>" onclick="fn_checkRow();"><span class="none"><spring:message code="board.check" /></span></label></td>
					<td><c:out value="${(paging.totalCount - (paging.currentPage -1) * paging.pageSize) - i.index}"/></td>
					<td><a class="link" href="javascript:fn_detailView('<c:out value="${item.termsId }"/>');" title="<spring:message code="board.detail"/>"><c:out value="${item.termsNm}"/></a></td>
					<td><c:out value="${item.termsDttm}"/></td>
				</tr>
			</c:forEach>
