...
<form name="boardSearchForm" class="board_search_form" method="post" autocomplete="off">
	<div class="search_box">
		<select name="so" title="<spring:message code="search.choose"/>">
			<option value="userId" <c:if test="${search.so eq 'userId'}">selected</c:if>><spring:message code="userIpVO.userId"/></option>
			<option value="ip" <c:if test="${search.so eq 'ip'}">selected</c:if>><spring:message code="userIpVO.ip"/></option>
		</select>		
...
<form:form id="listForm" name="listForm" method="post" autocomplete="off">
	...
	<table class="list_table">
		...
		<c:forEach var="item" items="${list }" varStatus="i">
			<tr>
				<%-- <td><label><input type="checkbox" name="checkRow" value="<c:out value="${item.userId }"/>#<c:out value="${item.ip }"/>" onclick="fn_checkRow();"><span class="none"><spring:message code="board.check" /></span></label></td> --%>
				<td><c:out value="${(paging.totalCount - (paging.currentPage -1) * paging.pageSize) - i.index}"/></td>
				<td><a class="link" href="javascript:fn_detailView('<c:out value="${item.userId }"/>', '<c:out value="${item.ip }"/>');" title="<spring:message code="board.detail"/>"><c:out value="${item.userId}"/></a></td>
...
<script type="text/javaScript" defer="defer">
	...
	//상세보기
	function fn_detailView(sId, sIp) {
